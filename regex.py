...
def find_word(string_list):
    ...
    #   \b[A-Za-z]+[0-9]{3}[A-Za-z]+\b
    # \b = boundary ; [A-Za-z]+ = at least one upper or lowercase;
    # [0-9]{3} = 3 numbers
    # initialize an empty list
    three_digit_words = []
    # define the regular expression
    lst = re.findall(
        '\S.+[0-9]\S.+')  #\S.+ is any num nonwhitespace, 0-9 is digits
    # loop through each line of the string list
    for line in string_list:
        line = line.rstrip()
        if re.search('\S.+[0-9]\S.+', line):
            three_digit_words.append(line)
...
class TestAllMethods(unittest.TestCase):
    ...
    def test_domains(self):
        ...
        self.assertEqual(domain_list, [
            'pythex.org', 'si.umich.edu', 'sabapivot.com',
            'stars.chromeexperiments.com', 'theofficestaremachine.com',
            'regex101.com'
        ])
